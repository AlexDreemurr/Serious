...
const calculateDimensions = () => {
  const containerWidth =
    document.getElementById("container").clientWidth || window.innerWidth - 20;
  ...
  if (window.innerWidth < 900) {
    ...
    margin = { top: 20, right: 15, bottom: 110, left: 30 };
  } else {
    // 桌面端：图表占75%宽度
...
const render = (data) => {
  ...
  bar
    ...
    .attr("height", (d) =>
      Math.max(0, innerHeight + margin.top - yScale(getScore(d)))
    )
    .merge(bar)
    .transition()
    .duration(1000)
...
d3.csv("./restaurant.csv").then((data) => {
  ...
  Object.keys(data[0]).forEach((d) => {
    if (d != "名称") {
      var div = controller.append("div").attr("class", "roll");
      div.append("p").text(d);
      div
        .append("input")
        .attr("id", d)
        ...
        .attr("step", "0.25")
        .attr("value", "0.5");
      div.append("p").text("0.5").attr("class", "weight");
    }
  });
  ...
  d3.selectAll("input[type='range']").on("input", function () {
    ...
    d3.select(this.parentNode).select(".weight").text(this.value);
    render(data);
  });
